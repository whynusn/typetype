...
class TestSystemIdentifier:
    ...
    def test_windows_short_circuit(self, monkeypatch):
        monkeypatch.setattr("src.backend.system_identifier.platform.system", lambda: "Windows")
        assert SystemIdentifier().get_system_info() == ("Windows", "N/A")

    def test_macos_short_circuit(self, monkeypatch):
        monkeypatch.setattr("src.backend.system_identifier.platform.system", lambda: "Darwin")
        assert SystemIdentifier().get_system_info() == ("macOS", "N/A")

    def test_linux_wayland_by_env(self, monkeypatch):
        monkeypatch.setattr("src.backend.system_identifier.platform.system", lambda: "Linux")
        monkeypatch.setattr(
    ...
    def test_linux_detect_by_loginctl_wayland(self, monkeypatch):
        monkeypatch.setattr("src.backend.system_identifier.platform.system", lambda: "Linux")
        monkeypatch.setattr("src.backend.system_identifier.os.environ", {"XDG_SESSION_ID": "1"})

        monkeypatch.setattr(
            "src.backend.system_identifier.subprocess.run",
            lambda *args, **kwargs: SimpleNamespace(returncode=0, stdout="Type=wayland\n"),
        )
    ...
    def test_linux_detect_by_loginctl_x11(self, monkeypatch):
        monkeypatch.setattr("src.backend.system_identifier.platform.system", lambda: "Linux")
        monkeypatch.setattr("src.backend.system_identifier.os.environ", {"XDG_SESSION_ID": "2"})
