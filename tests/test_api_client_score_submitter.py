...
🎓 异步提交测试策略：
- submit() 现在是异步的（入队操作），不直接调用 HTTP
- 测试 payload 结构：通过 SubmitTask.payload 验证
- 测试集成流程：启动队列 → 入队 → 等待队列清空 → 验证 HTTP 调用
"""
...
def test_build_payload_v2_pure_raw_contract():
    ...
    💡 测试策略：
    - 直接测试 SubmitTask.payload 属性（纯数据，无副作用）
    - 验证字段结构和值的正确性
    """
    ...
    # 通过 SubmitTask 构建 payload（纯数据验证，无需启动队列）
    task = SubmitTask(score_data=stat, text_id=123)
...
def test_async_submit_integration():
    """测试异步提交集成流程：入队 → 后台处理 → HTTP 调用。

    🎓 验证异步行为：
    - submit() 返回 True 表示成功入队（不等待 HTTP 响应）
    - 后台线程处理队列，调用 _do_http_submit
    - 通过等待队列清空来同步验证
    """
    ...
    try:
        ...
        # submit() 现在是异步的，返回 True 表示成功入队
        result = submitter.submit(stat, text_id=123)
        assert result is True, "submit() 应返回 True 表示成功入队"
