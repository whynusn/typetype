...
if __name__ == "__main__":
    ...
    log_debug(f"原文: {test_str}")
    log_debug(f"加密: {encrypted}")
    log_debug(f"解密: {decrypt(encrypted)}")

    # 测试加密对象
    test_obj = {"name": "Alice", "age": 25, "city": "Beijing"}
    encrypted_obj = encrypt(test_obj)
    log_debug(f"原文对象: {test_obj}")
    log_debug(f"加密: {encrypted_obj}")
    log_debug(f"解密: {decrypt(encrypted_obj)}")
    ...
    log_debug(f"解密: {decrypt(encrypted_str)}")
