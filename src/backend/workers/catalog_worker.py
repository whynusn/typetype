...
优先使用 OttTextProvider 获取目录（Phase 3），fallback 到
...
class CatalogWorker(BaseWorker):
    ...
    def __init__(
        ...
        registry_provider: "OttTextProvider | None" = None,
    ):
    ...
    def _fetch_catalog(self) -> list[dict]:
        ...
        优先使用 OttTextProvider.get_catalog()（Phase 3），
        fallback 到 LeaderboardGateway（兼容无开源文库场景）。
        """
        if self._registry_provider is not None:
            log_info("[CatalogWorker] 使用 OTT provider 获取目录...")
            catalog = self._registry_provider.get_catalog()
            log_info(
                f"[CatalogWorker] OTT provider 返回了 {len(catalog) if catalog else 0} 个来源"
            )
            if catalog:
                result = [
                    {
                ...
                log_info(
                    f"[CatalogWorker] 返回 {len(result)} 个来源给 LeaderboardAdapter"
                )
                return result
            log_info("[CatalogWorker] OTT provider 返回空目录，fallback 到 server API")
        ...
        log_info(
            f"[CatalogWorker] LeaderboardGateway 返回: {len(catalog) if catalog else 'None'}"
        )
        if catalog is None:
