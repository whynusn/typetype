...
FluentPage {
    ...
    property string initialSource: ""          // 外部传入，F2 打开时定位到自定义
    ...
    readonly property bool isRegistrySource: currentSource === "registry"
    ...
    function itemDisplayTitle() {
        ...
        return textLoadPanel.selectedSourceLabel || qsTr("自定义文本")
    }
    ...
    function itemDisplayCharCount() {
        ...
        return textLoadPanel.contentLength
    }

    function itemDisplayContent() {
        if (currentSource === "jisubei") return previewContent
        if (currentSource === "registry") return previewContent ? previewContent.substring(0, 200) : ""
        return ""
    }
    ...
    function progressIdentifier() {
        ...
        if (currentSource === "trainer") return trainerId(selectedItem)
        if (currentSource === "jisubei") return previewContent
        return textLoadPanel.contentText
    }
    ...
    function canLoad() {
        ...
        if (currentSource === "custom") return textLoadPanel.contentText.length > 0
    ...
    extraHeaderItems: [
        Row {
            ...
            BusyIndicator {
                width: 20; height: 20
                running: appBridge ? (appBridge.textListLoading || appBridge.localArticleLoading || appBridge.trainerLoading || registryLoading) : false
                visible: running
            }
        }
    ]
    ...
    ColumnLayout {
        ...
        GridLayout {
            ...
            StackLayout {
                id: leftStack
                Layout.fillHeight: true
                Layout.preferredWidth: root.wideMode ? Math.max(260, root.width * 0.35) : root.width
                Layout.maximumWidth: root.wideMode ? root.width * 0.45 : root.width
                currentIndex: root.currentSourceIndex
                ...
                TextSourceListPanel {
                    ...
                    loading: root.currentSource === "registry" && (appBridge ? appBridge.textListLoading : false)
                    emptyText: qsTr("暂无文本")
            ...
            Frame {
                ...
                ColumnLayout {
                    ...
                    TextInfoCard {
                        ...
                        visible: root.currentSource === "custom" ? textLoadPanel.contentText.length > 0
                                                                  : root.selectedItem !== null || root.itemDisplayTitle().length > 0
                    }
                    ...
                    SliceCriteriaPanel {
                        ...
                        Layout.fillWidth: true
                        visible: sliceSettingsPanel.sliceModeChecked
                    }
                    ...
                    RowLayout {
                        ...
                        Button {
                            Layout.preferredHeight: 34
                            text: root.currentSource === "registry" ? qsTr("载入跟打") : qsTr("载入跟打")
                            highlighted: true
    ...
    function appBridgeLoading() {
        if (!appBridge) return false
        return appBridge.textListLoading || appBridge.localArticleLoading || appBridge.trainerLoading || root.registryLoading
    }
