...
| Presentation | `Bridge` | QML 门面：属性代理、信号转发、Slot 入口；`baseUrl`/`setBaseUrl` 通过回调分发，不直接依赖 Integration |
| Presentation | `TypingAdapter` / `TextAdapter` / `AuthAdapter` / `CharStatsAdapter` / `LeaderboardAdapter` / `UploadTextAdapter` | Qt 适配、线程协调（所有 I/O 走 Worker）、错误回传；`TextAdapter.get_base_url()` 代理 RuntimeConfig 只读属性 |
