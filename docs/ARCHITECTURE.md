...
<<<<<<< Updated upstream
| 6 | `RinUI/windows/FluentPage.qml` | `container`（ColumnLayout）的 `anchors.top` + `anchors.horizontalCenter` 替换为 `y` + `x` 属性绑定 | ColumnLayout 同时使用 anchors 会导致子项触发 "Detected anchors on an item that is managed by a layout" 运行时警告；`x`/`y` 属性绑定功能等价且避免 anchors 与 Layout 机制冲突 |
=======
...
=======
|| 2026-04-19 | FluentPage `container` 的 anchors 替换为 x/y 属性绑定（消除 ColumnLayout 与 anchors 冲突警告）；SettingsPage ColumnLayout 的 `anchors.fill` 替换为 `Layout.fillWidth`/`Layout.fillHeight`（FluentPage 内容区子项不应使用 anchors） |
>>>>>>> Stashed changes
...
- **排行榜 Bug 修复**：基础已实现，存在 3 个未修复问题（详见 `docs/history/unfixed-bugs-2026-04-15.md`）
