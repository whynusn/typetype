...
| 当前文档 | 其他核心文档 | 快速链接 |
| :--- | :--- | :--- |
| **本文** — 架构设计、代码分层、数据流、陷阱 | [README.md](../README.md) — 快速入门<br>[AGENTS.md](../AGENTS.md) — 开发规范 | [快速开始](#快速开始)<br>[分层架构](#分层架构)<br>[已知陷阱](#已知陷阱) |
...
支持用户在设置界面运行时切换 API 服务器地址，修改后立即生效并持久化到 `config.json`。
...
**读取链路**（SettingsPage 初始化显示当前地址）：

```text
SettingsPage.qml
  -> appBridge.baseUrl (Property 绑定)
  -> Bridge.baseUrl
  -> TextAdapter.get_base_url()
...
| `ApiClientScoreSubmitter.update_base_url()` | 重算 `_submit_url` | — |
| `TextUploader.update_base_url()` | 重算 `_upload_url` | — |
| `LeaderboardFetcher.update_base_url()` | 更新 `_base_url` | — |
| `TextAdapter.get_base_url()` | 只读代理，供 Bridge 读取 | 不做写入 |

#### 设计要点

- **回调注入**：Bridge 不直接依赖 Integration 层对象（违反分层），通过 `base_url_update_callback` 闭包将变更请求"上抛"给装配层（`main.py`）处理
- **URL 清理**：`RuntimeConfig.update_base_url()` 会 `rstrip("/")` 去除尾部斜杠，所有 Integration 对象收到的都是清理后的 URL
- **持久化**：`RuntimeConfig._save_to_file()` 只覆盖 `base_url` 字段，保留其他配置不变
...
| `TextAdapter.get_base_url()` 代理只读属性给 Bridge | ✅ |
| `TextAdapter` 依据配置做业务决策 | ❌ |
| QML 直接读取配置文件做路由 | ❌ |
| Bridge 直接依赖 RuntimeConfig | ❌（通过 TextAdapter 代理或回调） |
| Bridge 直接持有 Integration 对象引用 | ❌（通过 Port 类型注解 + `base_url_update_callback` 闭包上抛到 main.py） |
...
| 3 | `RinUI/components/Navigation/NavigationView.qml` | 从 StackView 重构为单实例页面管理：移除 `StackView` 及其 push/pop 动画，改用 `pageInstances` 字典缓存页面实例，通过 `visible` + `active` 属性切换；`loggedin` 变化时同步给所有已创建实例；`setPropertySafe` 兼容无 `active`/`loggedin` 属性的页面 | StackView 每次导航 createObject 创建新实例，导致页面状态丢失、信号连接时序复杂（`StackView.status` 绑定延迟问题）。单实例模式页面创建一次后复用，状态由页面自身管理，NavigationView 只负责切换可见性 |
...
> 完整修改明细见 `RinUI/LOCAL_MODIFICATIONS.md`。
